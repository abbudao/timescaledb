...
--   qid_suffix    appended to every query id, '' for the normal pass
--   parallel_off  true -> max_parallel_workers_per_gather = 0 for this pass
--
-- run.sh calls this file twice per table when --parallel-off-pass is given:
-- once normally, once with parallel_off=true and qid_suffix='-np', so the
-- scorecard can tell an exclusion gain from free parallel workers.
--
...
SELECT probe_explain(:'run_id', :'variant', :'tbl', 'Q1' || :'qid_suffix', r, format(
...
SELECT probe_explain(:'run_id', :'variant', :'tbl', 'Q5' || :'qid_suffix', r, format(
